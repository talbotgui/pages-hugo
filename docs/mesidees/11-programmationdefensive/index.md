...
Mais une question se pose : "la programmation défensive doit-elle être utilisée partout ?". L'ajout de code vérifiant chaque paramètre de chaque méthode de chaque composant du système coûte du temps de développement (du code applicatif et des tests associés). De plus chaque ligne de code ajoute potentiellement un bug dans l'application. Et surtout la maintenabilité d'une application décroit avec l'augmentation du volume de code.
